...
@Component({
  selector: 'app-up-credito',
  templateUrl: './up-credito.component.html',
  styleUrls: ['./up-credito.component.css']
})
export class UpCreditoComponent implements OnInit, OnDestroy {
  ...
  guardaArchivos() {
    ...
    if (file !== undefined && file.name !== '') {
      ...
      reader.onload = () => {
        ...
        this._service.loadArchivos(this.fileDataOtros, 'pdf', 'Nota_Credito')
          .subscribe((data: any) => {
    ...
    if (fileOtros !== undefined && fileOtros.name !== '') {
      ...
      reader2.onload = () => {
        ...
        this._service.loadArchivos(this.fileDataOtros, 'otro', 'Nota_Credito')
          .subscribe((data: any) => {
            this.txtGuarda = 'Documento Guardado';
            this.disableGuardaArchvos = true;
            this.disableGuardaArch = true;
  ...
  validaDGT() {
    ...
    reader.onload = () => {
      ...
      this._service.loadXmlCredito(this.fileData)
        .subscribe((data: any) => {
          ...
          this.abilitar.emit();
          localStorage.setItem('flujo', 'credito');
        },
          error => {
            this.respuestaDgt.descStatus = error.error.data.mensaje;
            this.txtvalidaDGT = 'Validar';
