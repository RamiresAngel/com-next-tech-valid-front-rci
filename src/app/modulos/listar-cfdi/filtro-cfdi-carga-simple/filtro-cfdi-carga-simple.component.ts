...
import { CorporativoActivo, Contribuyente, Usuario, Sucursal, Movimientos, Departamento, TipoGasto, FiltroCFDICS } from 'src/app/entidades';
...
@Component({
  selector: 'app-filtro-cfdi-carga-simple',
  templateUrl: './filtro-cfdi-carga-simple.component.html',
  styleUrls: ['./filtro-cfdi-carga-simple.component.css']
})
export class FiltroCfdiCargaSimpleComponent implements OnInit {
  @Output() filtrar = new EventEmitter();

  public filtroConsulta = new FiltroCFDICS();
  ...
  limpiarCampos() {
    ...
    this.filtroConsulta = new FiltroCFDICS();
  ...
  onDepartamentoSelected(obj: any) {
    if (obj.value !== null && obj.value !== '' && obj.value !== '0') {
      this.formulario_filtro.get('departamento').setValue(obj.value);
    } else {
      this.filtroConsulta.identificador_departamento = '';
      this.formulario_filtro.get('departamento').setValue(null);
    }
  }
  onTipoGastoSelected(obj: any) {
    if (obj.value !== null && obj.value !== '' && obj.value !== '0') {
      this.formulario_filtro.get('tipo_gasto').setValue(obj.value);
      this.filtroConsulta.tipo_gasto = obj.value;
    } else {
      this.filtroConsulta.tipo_gasto = 0;
      this.formulario_filtro.get('tipo_gasto').setValue(null);
    }
  }
  onTipoComprobanteSelected(obj: any) {
    if (obj.value !== null && obj.value !== '' && obj.value !== '0') {
      this.formulario_filtro.get('tipo_comprobante').setValue(obj.value);
      this.filtroConsulta.tipo_comprobante = obj.value;
    } else {
      this.filtroConsulta.tipo_comprobante = '';
