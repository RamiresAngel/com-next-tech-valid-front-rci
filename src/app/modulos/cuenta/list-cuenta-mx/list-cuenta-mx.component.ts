...
@Component({
  selector: 'app-list-cuenta-mx',
  templateUrl: './list-cuenta-mx.component.html',
  styleUrls: ['./list-cuenta-mx.component.css']
})
export class ListCuentaMxComponent implements OnInit {
  ...
  actualizarTablaCuenta(identificador_corporativo: any = null) {
    ...
    this._cuentaService.obtenerCuentaCorporativo(identificador_corporativo).subscribe((data: HttpResponse<Cuenta[]>) => {
      ...
      setTimeout(() => {
        $('#tabla_cuenta').DataTable(this.opcionesDt);
      }, 1000);
    });
  }
