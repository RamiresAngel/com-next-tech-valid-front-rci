...
import { EnviarAcuse, FacturaDgt, Cliente } from '../../../entidades/acuse';
...
export class NotasDebitoComponent implements OnInit {
  ...
  dtOptions: any = {};
  constructor(
    private _notasDebitoService: NotasDebitoService,
  ...
  funcionFacturas() {
    ...
    // const urlReportExcel = this.urlReportExcel;
    const that = this;
    this.dtOptions = {
      ...
      createdRow: function (row, data, index) {
        ...
        let link =
          '<a href="https://api.valid.next-technologies.co.cr/api/v1/valid/manage_file/file/pdf/nota_debito/' +
          data.id +
          '" target="_blank" title="PDF"><i class="fa fa-file-pdf"></i></a>';
        // tslint:disable-next-line:max-line-length
        link +=
          '&nbsp; <a href="https://api.valid.next-technologies.co.cr/api/v1/valid/manage_file/file/xml/nota_debito/' +
          data.id +
  ...
  doctsRelacionadosdo(data: any) {
    ...
    if (this.doctsRelacionados.length === 0) {
      this.alerta = 'Esta Nota de Débito no cuenta con documentos relacionados';
    } else {
  ...
  obtenerEntidadReceptor(data: any) {
    ...
    /* this.acuse.factura.tipo_timbrado = 'TEST';
        this.acuse.factura.CLAVE_FACTO = 'FHCRP'; */
    ...
    this._enviarDgt.enviarAcuse(this.acuse).subscribe(
      ...
      error => {
        this.alerta = '';
        this.alerta = 'Ocurrio un error  ' + error;
