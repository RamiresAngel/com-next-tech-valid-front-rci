...
export class ModalConceptosComprobantesComponent implements OnInit, OnChanges {
  ...
  calcularMontoReembolsarConcepto(concepto, bote_disponible: number, porcentaje_prestacion: number, total_factura: number, index_concepto: number = null): number {
    if (concepto.aplica) {
    ...
    return concepto.monto_rembolsar;
  }
    ...
    if (this.controlsConceptos[i].controls.monto_rembolsar.value > (this.controlsConceptos[i].controls.importe.value * this.controlsConceptos[i].controls.tipo_cambio.value)) {
      if (this.tipo_gasto == 11) {
        if (this.monto_disponible === 0) {
          ...
          if (this.comprobante.conceptos[i].impuestos) {
            const impuestos = [this.comprobante.conceptos[i].impuestos];
  ...
  calcularImporte(i) {
    try {
      ...
      if (this.tipo_gasto == 11) {
        let monto_rembolsar = this.controlsConceptos[i].controls.importe.value * this.controlsConceptos[i].controls.tipo_cambio.value;
        let impuestos = 0;
        let impuestos_retenciones = 0;
        let impuestos_traslados = 0;
        if (this.comprobante.conceptos[i].impuestos && this.comprobante.conceptos[i].impuestos.retenciones) {
          this.comprobante.conceptos[i].impuestos.retenciones.forEach(retencion => {
            impuestos_retenciones = impuestos_retenciones + retencion.importe;
          });
        }
        if (this.comprobante.conceptos[i].impuestos && this.comprobante.conceptos[i].impuestos.traslados) {
          this.comprobante.conceptos[i].impuestos.traslados.forEach(traslados => {
            impuestos_traslados = impuestos_traslados + traslados.importe;
          });
        }
        ...
        this.controlsConceptos.forEach(concepto => {
          if (concepto.controls.aplica.value) {
