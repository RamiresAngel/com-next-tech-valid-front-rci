...
export class ModalComprobanteComponent implements OnInit {
  ...
  configurarTabla(): void {
    ...
    this.tabla = $('#example').DataTable({
      ...this.globals.dtOptions, ...{
        "data": this.lista_comprobantes,
        "columns": [
          {
    ...
    $('#example tbody').prop('onclick', null).off('click');
  ...
  format(d) {
    ...
    <tr>
      ...
      <td> ${concepto.monto_rembolsar} </td>
      <td> ${concepto.observacion} </td>
      <td>  <input type="checkbox" checked="${concepto.comprobante_fiscal == 0 ? false : true}"/></td>
      <td> ${concepto.aplica} </td>
    </>
    `);

    let tabla_conceptos = '<table id="#tabalConceptos" class="table">' + header + '<tbody> ';
    d.conceptos.forEach(concepto => {
      tabla_conceptos = tabla_conceptos + row(concepto);
    });
    tabla_conceptos = tabla_conceptos + ' </tbody> </table>';
