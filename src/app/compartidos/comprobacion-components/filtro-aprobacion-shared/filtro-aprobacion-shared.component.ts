...
import { Usuario, CorporativoActivo } from 'src/app/entidades';
...
@Component({
  selector: 'app-filtro-aprobacion-shared',
  templateUrl: './filtro-aprobacion-shared.component.html',
  styleUrls: ['./filtro-aprobacion-shared.component.css']
})
export class FiltroAprobacionSharedComponent implements OnInit {
  ...
  lista_aprobadores = new Array<any>();
  ...
  listJefeAsistidos() {
    this._comprobacionService.getUsuarioByAsistente(this.usuario.identificador_usuario)
      .subscribe((data) => {
        this.lista_aprobadores = $.map(data, (obj) => {
  ...
  limpiarSelects() {
    ...
    // const asistido = this.lista_aprobadores;
    ...
    setTimeout(() => {
      ...
      // this.lista_aprobadores = asistido;
    }, 200);
  }
